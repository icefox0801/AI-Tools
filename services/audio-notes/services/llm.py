...
def chat_with_context_streaming(
    message: str,
    history: list,
    transcript: str,
    summary: str = "",
    ...
    """
    Chat with Ollama using transcript and summary as context (streaming version).
    ...
    logger.info(
        f"chat_with_context_streaming - transcript len: {len(transcript) if transcript else 0}, summary len: {len(summary) if summary else 0}"
    )
...
def chat_with_context(
    ...
    logger.info(
        f"chat_with_context called - transcript len: {len(transcript) if transcript else 0}, summary len: {len(summary) if summary else 0}"
    )
