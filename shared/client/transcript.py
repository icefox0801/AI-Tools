...
def parse_segment_id(segment_id: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    if '-' in segment_id and segment_id.count('-') == 1:
        ...
        if len(parts) == 2:
            ...
            if (start.startswith('s') and start[1:].isdigit() and
                end.startswith('s') and end[1:].isdigit()):
                return (start, end)
...
def get_segment_number(segment_id: str) -> Optional[int]:
...
class TranscriptManager:
    ...
    def update(self, segment_id: str, text: str) -> bool:
        ...
        if start_id != end_id and start_num is not None and end_num is not None:
            ...
            ids_to_remove = []
            for sid in self._segments.keys():
    ...
    def get_last_segment(self) -> Optional[tuple]:
        """
