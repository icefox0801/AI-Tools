...
class ASRResult:
    ...
    @classmethod
    def from_partial(cls, text: str, chunk_ids: list[str] | None = None) -> "ASRResult":
        """Create a partial result."""
        return cls(text=text, is_final=False, chunk_ids=chunk_ids or [])

    @classmethod
    def from_final(
        cls, text: str, chunk_ids: list[str] | None = None, confidence: float = 1.0
    ) -> "ASRResult":
        """Create a final result."""
        return cls(text=text, is_final=True, confidence=confidence, chunk_ids=chunk_ids or [])

    @classmethod
    def from_error(cls, error: str) -> "ASRResult":
        """Create an error result."""
        return cls(text="", is_final=True, success=False, error=error)
    ...
    def __str__(self) -> str:
        ...
        return (
            f"ASRResult({status}: {self.text[:50]}...)"
            if len(self.text) > 50
            else f"ASRResult({status}: {self.text})"
        )
