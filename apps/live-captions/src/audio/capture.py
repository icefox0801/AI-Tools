...
class MicrophoneCapture(AudioCapture):
    ...
    def __init__(self, callback: Callable[[bytes], None], device_index: int | None = None):
        """
    ...
    def start(self) -> bool:
        try:
            ...
            is_stereo_mix = (
                "立体声混音" in self._device_name or "stereo mix" in self._device_name.lower()
            )
            ...
            logger.info(
                f"Rate: {native_rate}Hz → {TARGET_SAMPLE_RATE}Hz, Channels: {self.capture_channels}"
            )
    ...
    def start(self) -> bool:
        try:
            ...
            if not loopback_device:
                raise RuntimeError(
                    "No loopback device found. Install PyAudioWPatch: pip install pyaudiowpatch"
                )
            ...
            logger.info(
                f"Rate: {self.capture_rate}Hz → {TARGET_SAMPLE_RATE}Hz, Channels: {channels}"
            )
